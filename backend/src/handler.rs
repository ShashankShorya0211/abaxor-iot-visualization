...
//! endpoints because our autogenerated `reg_bank.rs` doesn't provide generic
...
/// JSON payload for writing DAC register 196
#[derive(Debug, Deserialize)]
pub struct RegisterWriteReq {
    /// Value interpreted as an unsigned 32-bit integer (decimal or hex on the frontend)
    pub value: u32,
}

/// JSON payload describing a register value we send back to the frontend
#[derive(Debug, Serialize)]
struct RegisterValueResp {
    register: &'static str,
    register_id: u16,
    value: u32,
    hex: String,
...
impl RegisterValueResp {
    fn new(register: &'static str, register_id: u16, value: u32, hex: String) -> Self {
        Self {
            register,
            register_id,
            value,
...
fn reg196_payload(value: u32) -> RegisterValueResp {
    RegisterValueResp::new("dac_ch0_msb", 196, value, format!("0x{value:08X}"))
}
...
#[get("/api/registers/dac/ch0/msb")]
pub async fn read_dac_ch0_msb() -> impl Responder {
    let (reg_fd, _) = match open_devices() {
        Ok(fds) => fds,
        Err(e) => return e,
    };

    let value = reg_bank::get_dac_value_ch00_msb(reg_fd);
    HttpResponse::Ok().json(reg196_payload(value))
}

#[post("/api/registers/dac/ch0/msb")]
pub async fn write_dac_ch0_msb(body: web::Json<RegisterWriteReq>) -> impl Responder {
    let (reg_fd, _) = match open_devices() {
        Ok(fds) => fds,
        Err(e) => return e,
    };

    match reg_bank::set_dac_value_ch00_msb(reg_fd, body.value) {
        Ok(_) => HttpResponse::Ok().json(reg196_payload(body.value)),
...
pub fn config(cfg: &mut web::ServiceConfig) {
    cfg.service(health)
        ...
        .service(stop_stream)
        .service(stream_once)
	.service(read_dac_ch0_msb)
	.service(write_dac_ch0_msb);
}
